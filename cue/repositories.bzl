...
_ATTRS = {
    "cue_version": attr.string(mandatory = True, values = TOOL_VERSIONS.keys()),
    "platform": attr.string(mandatory = True, values = PLATFORMS.keys()),
}

def _cue_repo_impl(repository_ctx):
    url = "https://github.com/someorg/someproject/releases/download/v{0}/cue-{1}.zip".format(
        repository_ctx.attr.cue_version,
        repository_ctx.attr.platform,
    )
    repository_ctx.download_and_extract(
        url = url,
        integrity = TOOL_VERSIONS[repository_ctx.attr.cue_version][repository_ctx.attr.platform],
    )
    build_content = """# Generated by cue/repositories.bzl
load("@abc_dp_rules_cue//cue:toolchain.bzl", "cue_toolchain")

cue_toolchain(
    name = "cue_toolchain",
    target_tool = select({
        "@bazel_tools//src/conditions:host_windows": "cue_tool.exe",
        "//conditions:default": "cue_tool",
    }),
)
"""
...
def cue_register_toolchains(name, register = True, **kwargs):
